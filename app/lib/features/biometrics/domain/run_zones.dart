...
/// De onde vieram restingBpm/maxBpm efetivos. UI usa pra mostrar badge
/// "ESTIMADO" quando saiu do fallback 220-idade ou do summary observado.
enum BpmRangeSource { profile, derived }

class BpmRange {
  final int? resting;
  final int? max;
  final BpmRangeSource source;
  const BpmRange({this.resting, this.max, required this.source});

  bool get isValid => resting != null && max != null && max! > resting!;
}

/// Cascata pros valores efetivos de resting/maxBpm:
///   resting: profile.restingBpm → summary.avgRestingBpm
///   max:     profile.maxBpm → (220 - idade) → summary.maxBpm observado
