...
export interface CoachChatBuildInput {
...
export async function buildCoachChatPrompt(args: CoachChatBuildInput): Promise<BuiltPrompt> {
  const { config, source } = await getPromptConfig('coach-chat');
